<template>
    ...
    <div v-if="data" class="app flex justify-center" style="padding: 35px "> 
          <div v-for="rating in data.ratings" :key="rating.id">
            <div class=" bg-opacity-75 mx-5 px-16 rounded-lg bg-gray-600 shadow-2xl overflow-hidden">
...
export default createFrontend({
  ...
  methods: {
    getData() {
      fetch("https://ow-api.com/v1/stats/" + this.platform + "/" + this.region + "/" + this.battleTag.replace("#","-") + "/profile", {
      })
...
@keyframes glow {
  from {
    text-shadow: 0 0 5px #fff, 0 0 10px #fff, 0 0 20px #0092e6, 0 0 30px #0004e6, 0 0 40px #0004e6, 0 0 50px #0004e6, 0 0 60px #0004e6;
  }
  to {
    text-shadow: 0 0 10px #fff, 0 0 20px #0092e6, 0 0 30px #0092e6, 0 0 40px #0092e6, 0 0 50px #0092e6, 0 0 60px #0092e6, 0 0 70px #0092e6;
  }
}
